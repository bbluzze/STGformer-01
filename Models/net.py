...
class GNNStack(nn.Module):
    ...
    def forward(self, inputs: Tensor):
        ...
        # Ensure num_nodes matches x's node dimension
        num_nodes = x.size(2)  # Get the number of nodes from x

        adj = self.g_constr(x.size(0), x.size(1), num_nodes, x.device)
        ...
        for tconv, gconv, bn, pool in zip(self.tconvs, self.gconvs, self.bns, self.diffpool):
            print(f"x.shape before tconv: {x.shape}")
            print(f"adj.shape before gconv: {adj.shape}")

            x, adj = pool(gconv(tconv(x), adj), adj)

            print(f"x.shape after tconv and gconv: {x.shape}")
            print(f"adj.shape after pool: {adj.shape}")
